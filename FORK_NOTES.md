...
| B3 | FORK (feature branch, **unmerged**) | superproject | FOC/SINE build (not byte-identical — feature addition, expected); host suite green incl. 2 new VCU cases; **NO HW PASS** | on `feat/B3-limit-reasons` — port of upstream PR #59 (accel/regen power-limit reasons). Publishes `acclimreason`/`regenlimreason` spot values (ids **2061/2062** — renumbered off upstream's 2058/2059, which B2-autotune reserves at 2058–2060) naming the derate that would bite at full `throtmax`/`throtmin` demand. Adapted to this fork's throttle: upstream splits the accel/freq limiters into state-update + apply-only halves; replicated here but preserving our **T12** two-filter frequency path — `AccelerationLimitCommand`'s function-local statics lifted to members (`accelSpeed`/`accelSpeedDiff`) + new `ApplyAccelerationLimit`; `RunFrequencyLimit` replaced by a shared `ApplyFreqLimitAt(finalSpnt, filtered)` clamp with `FrequencyLimitCommand`/`FrequencyLimitCommandFw` each still owning their own IIR state (`frqFiltered`/`fwFrqFiltered`), plus a member-reading `ApplyFrequencyLimit` for the probe. `GetPowerLimitReason` (vehiclecontrol.cpp) dry-recomputes OUR exact ProcessThrottle chain — BMS(!Tesla)/UDC/IDC/FMAX/ACCEL/TMPHS/TMPM — on a copy of the setpoint, using the apply-only variants so the probe never disturbs the live IIR state (the real chain updated it once this tick). **Deliberately drops upstream's `8=RegenRamp`**: this fork replaced the upstream regen ramp with the stateful `regenTaperHold` (**F4**), which runs *after* `potnom` is published — outside the recomputable chain — so a RegenRamp reason could never be honestly attributed; `LIMREASONS` + the `limitreasons` enum stop at `7=Tmpm`. Host: 2 new VCU cases (None + Derated) traced against our defaults (udcmin 450 / udcmax 520 / tmphsmax 85; `TemperatureDerate = (tempMax−temp)×10`); shared `TestCaseSetup` now seeds the limit-chain `Throttle` statics + `ResetDerateState()` (`Param::Change` is stubbed in the harness) — existing VCU/throttle/T12 cases stay green. **Marginal for the dyno itself** — `manualiq` bypasses these derates, so it only aids normal-torque-path validation runs. **RULED 2026-07-20 (call delegated by Jotham): stays a side branch** — marginal for the dyno (`manualiq` bypasses these derates); revisit only if normal-torque-path validation runs start needing derate attribution. Branch kept pushed to `fork` for that eventuality. |
