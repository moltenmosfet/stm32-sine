...
{
   // 530V exceeds the regen UDC limit (udcmax 520 -> Throttle::udcmax 525.2),
   // while 80°C against an 85°C limit derates accel to 50% ((85-80)*10).
...
void VCUTest::TestCaseSetup()
{
   ...
   Param::LoadDefaults();
   //Param::Change is stubbed out in this harness, so the Throttle:: statics that
   //firmware normally derives from params stay at their static-init values.
   //Seed the limit-chain ones (mirroring the firmware's derivations) so the
   //derate/limit-reason paths run against real defaults. (B3)
